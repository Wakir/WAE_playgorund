...
if __name__ == '__main__':
    ...
    chunk_classifiers = [15, 10, 5, 3, 1]
    #chunk_classifiers2 = [3]
    ...
    if args.parallel:
        Parallel(n_jobs=-1)(
            ...
            ({'streams_random_seeds': random_seeds, 'ensembles': (new_WAE(base_estimator=base_classifier, scale=scale, n_classifiers=n, base_quality_measure=quality_measure, pruning_criterion=quality_measure),),
              ...
              'metrics': metrics,
              'imbalance': [1 - imbalance1, imbalance1], 'gradual': True,
              'file': os.path.join(path, f'Stream_gradual_drift_{str(100 * imbalance1)}_imbalance_new_wae_{str(scale)}_{str(n)}_cl.npy')}
        ...
        if args.reference:
            ...
            Parallel(n_jobs=-1)(
                ...
                ({'streams_random_seeds': random_seeds,
                  ...
                  'ensembles_labels': (ref.__name__,),
                  'metrics': metrics,
                  'imbalance': [1 - imbalance2, imbalance2], 'gradual': False,
                  'file': os.path.join(path,
        ...
        for scale in scales:
            for n in chunk_classifiers:
                conduct_experiment(random_seeds,
                                    ...
                                    (f"new_wae_{str(scale)}_{str(n)}_cl",), metrics, [1 - imbalance1, imbalance1], True,
                                    os.path.join(path,
                                                f'Stream_gradual_drift_{str(100 * imbalance1)}_imbalance_new_wae_{str(scale)}_{str(n)}_cl.npy')
                                    )
                """conduct_experiment(random_seeds,
                                    (new_WAE(base_estimator=base_classifier, scale=scale, n_classifiers=n, base_quality_measure=quality_measure),),
