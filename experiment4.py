...
from strlearn2.ensembles import pruning
...
if __name__ == '__main__':
    ...
    quality_measure = bac
    pruner_types = [pruning.MultipleOffBestPruner, pruning.MultipleOffPruner, pruning.GeneticPruning]
    pruning_names = ["MOBP", "MOP", "GP"]
    ...
    if args.parallel:
        Parallel(n_jobs=-1)(
            ...
            for kwargs in
            ({'streams_random_seeds': random_seeds, 'ensembles': (new_WAE(base_estimator=base_classifier, scale=scale, n_classifiers=n, base_quality_measure=quality_measure, pruning_criterion=quality_measure, pruner_type=pruner_types[p]),),
              'ensembles_labels': (f"new_wae_{str(pruning_names[p])}_cl",),
              'metrics': metrics,
              'imbalance': [1 - imb, imb], 'gradual': True,
              'file': os.path.join(path, f'Stream_gradual_drift_{str(100 * imb)}_imbalance_new_wae_{str(pruning_names[p])}_cl.npy')}
             for p in range(len(pruner_types)))
             )
        Parallel(n_jobs=-1)(
            delayed(conduct_experiment)(**kwargs)
            for kwargs in
            ({'streams_random_seeds': random_seeds,
              'ensembles': (new_WAE(base_estimator=base_classifier, scale=scale, n_classifiers=n, base_quality_measure=quality_measure, pruning_criterion=quality_measure, pruner_type=pruner_types[p]),),
              'ensembles_labels': (f"new_wae_{str(pruning_names[p])}_cl",),
              'metrics': metrics,
              'imbalance': [1 - imb, imb], 'gradual': False,
              'file': os.path.join(path,
                                   f'Stream_sudden_drift_{str(100 * imb)}_imbalance_new_wae_{str(pruning_names[p])}_cl.npy')}
             for p in range(len(pruner_types)))
        )
        if args.reference:
            Parallel(n_jobs=-1)(
